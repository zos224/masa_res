    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                ...
                <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md"><Link href={"/admin/event/create"}>Add Event</Link></button>
            </div>
            {!loading ? (
                    ...
                    <tbody>
                        {events.map((event, index) => (
                            <tr key={event.id} className={`bg-white ${index == events.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4">
                                    {moment(event.startDate).format('YYYY-MM-DD hh:mm A')}
                                </td>
                                ...
                                <td className="px-6 py-4">
                                    <button onClick={() => {setOpenModal(!openModal); setDeleteLink("/api/event/" + event.id)}} className="hover:text-boxdark-2 hover:font-bold">Delete</button>
                                </td>
                            </tr>
