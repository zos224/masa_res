...
const EventPage = () => {
    ...
    useEffect(() => {
        const fetchEvent = async () => {
            const res = await fetch('/api/reservation/group')
            if (res.ok) {
    ...
    const onDeleted = (deleted) => {
        if (deleted) {
            ...
            const newGroups = groups.filter((e) => e.id != id)
            setGroups(newGroups)
        }
    }
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                <h2 className="font-bold text-xl">Group Reservation Management</h2>
            </div>
            {!loading ? (
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-white">
                    ...
                    <tbody>
                        {groups.map((group, index) => (
                            <tr key={group.id} className={`bg-white ${index == groups.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4 overflow-hidden">
                                    {group.reservation.numberOfGuests}
                                </td>
                                <td className="px-6 py-4">
                                    {moment(group.reservation.dateTime).format('YYYY-MM-DD hh:mm A')}
                                </td>
                                <td className="px-6 py-4">
                                    <button onClick={() => setCurrentGroup(index)} className="hover:text-boxdark-2 hover:font-bold">View</button>
                                </td>
                                <td className="px-6 py-4">
                ...
                <ViewReservation type={"group"} data={groups[currentGroup]} isOpen={viewModal} onClose={() => {setViewModal(false); setCurrentGroup(-1)}}></ViewReservation>
            </div>
            ) : (
