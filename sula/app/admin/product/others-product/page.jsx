...
const OthersProductPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchCategory = async () => {
            const res = await fetch("/api/subtype/notype")
            if (res.ok) {
                const data = await res.json()
                setCategories(data)
            }
    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                <h2 className="font-bold text-xl">Others Product Management</h2>
            </div>
            {!loading ? (
                <div>
                    ...
                    <div className="mt-10 w-full relative overflow-x-auto shadow-md sm:rounded-lg ">
                        <div className="flex justify-between mx-4">
                            ...
                            <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md" onClick={() => {setCurrentCategory({id: null, name: ''}); setAction("create"); setOpenModalCategory(true)}}>Add Category</button>
                        </div>
                        ...
                        <ModalParent isOpen={openModalCategory} onClose={(category) => {setOpenModalCategory(false); if (category) updateOrInsertCategory(category)}} action={action} data={currentCategory} name={"sub type"}></ModalParent>
                    </div>
