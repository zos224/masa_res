    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                ...
                <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md"><Link href={"/admin/product/products/create"}>Add Product</Link></button>
            </div>
            {!loading ? (
                    ...
                    <tbody>
                        {products.map((product, index) => (
                            <tr key={product.id} className={`bg-white ${index == products.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4">
                                    <Link className="hover:text-boxdark-2 hover:font-bold" href={"/admin/product/products/update/" + product.id}>Update</Link>
                                </td>
                                <td className="px-6 py-4">
                                    <button onClick={() => {setOpenModal(!openModal); setDeleteLink("/api/product/" + product.id)}} className="hover:text-boxdark-2 hover:font-bold">Delete</button>
                                </td>
                            </tr>
