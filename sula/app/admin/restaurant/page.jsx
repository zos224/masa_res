    ...
    return (
        <div className="w-full">
            <div className="flex justify-between">
                ...
                <button className="dark:bg-bodydark dark:text-black-2 text-black bg-white rounded-md px-3 py-2 shadow-md"><Link href={"/admin/restaurant/create"}>Add Restaurant</Link></button>
            </div>
            {!loading ? (
                ...
                <table className="w-full text-sm text-center text-gray-500 dark:text-white">
                    ...
                    <tbody>
                        {restaurants.map((restaurant, index) => (
                            <tr key={restaurant.id} className={`bg-white ${index == restaurants.length - 1 ? "" : "border-b"} dark:bg-bodydark text-black border-body`}>
                                ...
                                <td className="px-6 py-4">
                                    <Link className="hover:text-boxdark-2 hover:font-bold" href={"/admin/restaurant/update/" + restaurant.id}>Update</Link>
                                </td>
                                <td className="px-6 py-4">
                                    <button onClick={() => {setOpenModal(!openModal); setDeleteLink("/api/restaurant/" + restaurant.id)}} className="hover:text-boxdark-2 hover:font-bold">Delete</button>
                                </td>
                            </tr>
