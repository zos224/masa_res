...
const ModalParent = ({isOpen, onClose, action, data, name}) => {
    const [submitting, setSubmitting] = useState(false)
    const [errorAlert, setErrorAlert] = useState(null)
    const [type, setType] = useState({
        id: data.id,
        name: data.name,
        idMenu: data.idMenu ? data.idMenu : null
    })
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append("idMenu", type.idMenu)
                        ...
                        <div class="px-4 lg:px-10 py-10 w-full ">
                            <form onSubmit={handleSubmit}>
                                <div class="w-full px-4 mx-auto">
                                    ...
                                    <input value={type.name} type="text" placeholder={`Input name of ` + name} onChange={(e) => {setType({...type, name: e.target.value})}}  class="border-0 px-3 py-3 placeholder-bodydark2 text-black dark:bg-bodydark bg-white rounded text-sm shadow-4 focus:outline-none focus:ring w-full ease-linear transition-all duration-150" />
                                    </div>
                                </div>
                                {!submitting ? (
