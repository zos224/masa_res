...
const ModalManyToMany = ({isOpen, onClose, action, childData, listParent1, listParent2, name, parent1, parent2}) => {
    const [submitting, setSubmitting] = useState(false)
    const [errorAlert, setErrorAlert] = useState(null)
    const [parents1, setParents1] = useState(listParent1)
    const [parents2, setParents2] = useState(listParent2)
    useEffect(() => {
        setParents1(listParent1)
    }, [listParent1])

    useEffect(() => {
        setParents2(listParent2)
    }, [listParent2])

    const [child, setChild] = useState({
        id: childData.id,
        idParent1: childData.idParent1,
        idParent2: childData.idParent2
    })
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append("idParent1", child.idParent1)
        formData.append("idParent2", child.idParent2)
                            ...
                            <form onSubmit={handleSubmit}>
                                <div class="w-full px-4 mx-auto">
                                    <div class="relative w-full mb-3">
                                        ...
                                        <select value={child.idParent1} required onChange={(e) => setChild({...child, idParent1: e.target.value})} className="text-black border-0 px-3 py-3 dark:bg-bodydark bg-white rounded text-sm shadow-4 focus:outline-none focus:ring w-full ease-linear"> 
                                            {parents1.map((p, index) => (
                                ...
                                <div class="w-full px-4 mx-auto">
                                    <div class="relative w-full mb-3">
                                        ...
                                        <select value={child.idParent2} required onChange={(e) => setChild({...child, idParent2: e.target.value})} className="text-black border-0 px-3 py-3 dark:bg-bodydark bg-white rounded text-sm shadow-4 focus:outline-none focus:ring w-full ease-linear"> 
                                            {parents2.map((p, index) => (
                                                <option className="dark:text-black" key={index} value={p.id}>{p.name}</option>
                                            ))}
                                        </select>
                                    </div>
...
export default ModalManyToMany